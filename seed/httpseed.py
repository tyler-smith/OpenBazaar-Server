...
def run(*args):
    ...
    def start_server(keychain, first_startup=False):
        # logging
        logFile = logfile.LogFile.fromFullPath(os.path.join(DATA_FOLDER, "debug.log"), rotateLength=15000000, maxRotatedFiles=1)
        ...
        keys_file = os.path.join(DATA_FOLDER, "keys.pickle")
        if os.path.isfile(keys_file):
            keys = pickle.load(open(keys_file, "r"))
